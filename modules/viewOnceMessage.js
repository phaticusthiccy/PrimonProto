...
addCommand({ pattern: "^show$", access: "all", desc: "_It allows you to view the view once messages._" }, async (msg, match, sock, rawMessage) => {
    ...
    if (msg.quotedMessage?.viewOnceMessage || msg.quotedMessage?.viewOnceMessageV2 || msg.quotedMessage?.viewOnceMessageV2Extension) {
        ...
        var configs = {
            _0: viewOnceMessage?.imageMessage ? viewOnceMessage.imageMessage : viewOnceMessage?.videoMessage,
            _1: viewOnceMessage?.imageMessage ? "image" : "video",
            _2: mediaPath
        };
